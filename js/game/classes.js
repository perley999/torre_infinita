  ...
  function showSpecChangeConfirm() {
    ...
    for (const spec of specs) {
      ...
      btn.onclick = () => {
        ...
        `;
        grid.innerHTML = `
          <button class="choice-btn" onclick="Game.classes.confirmSpecChange('${spec.id}')">
            <span class="title" style="color:#4ade80;">✅ Sí, cambiar a ${spec.name}</span>
          </button>
          <button class="choice-btn" onclick="Game.classes.showSpecChangeConfirm()">
  ...
  function showClassChangeConfirm() {
    ...
    for (const cls of Object.values(CLASS_CONFIG)) {
      ...
      btn.onclick = () => {
        ...
        `;
        grid.innerHTML = `
          <button class="choice-btn" onclick="Game.classes.confirmClassChange('${cls.id}')">
            <span class="title" style="color:#4ade80;">✅ Sí, cambiar a ${cls.name}</span>
          </button>
          <button class="choice-btn" onclick="Game.classes.cancelClassChange()">
  ...
  function renderCharacterScreen() {
    ...
    html += `</div>
      <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;">
        ${metaState.playerClass && metaState.playerSpec ? `<button onclick="Game.classes.showSpecChangeConfirm()" style="flex:1;min-width:120px;background:transparent;border:1px solid #a78bfa;color:#a78bfa;border-radius:8px;padding:10px;cursor:pointer;font-weight:600;">⚡ Cambiar especialización</button>` : ''}
        ${metaState.playerClass ? `<button onclick="Game.classes.showClassChangeConfirm()" style="flex:1;min-width:120px;background:transparent;border:1px solid #ef4444;color:#ef4444;border-radius:8px;padding:10px;cursor:pointer;font-weight:600;">🔄 Cambiar clase</button>` : ''}
        <button onclick="Game.classes.closeCharacterScreen()" style="flex:1;min-width:120px;background:#3b82f6;color:white;border:none;border-radius:8px;padding:10px;cursor:pointer;font-weight:600;">🔙 Volver</button>
      </div>
    `;
  ...
  function renderRuneCharacterSection() {
      ...
      if (item && item.runas && item.runas.conditionId) {
        ...
        runeHTML += `
          <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 8px;margin-bottom:4px;background:#111118;border-radius:8px;border:1px solid #2a2a4a;">
            ...
            <button class="rune-action-btn danger" onclick="Game.classes.doUnequipRune('${slot}')" style="font-size:0.7rem;">Desequipar</button>
          </div>
        `;
      }
    }
    ...
    return `
      <div style="background:#1a1a2e;border-radius:12px;padding:12px;border:1px solid #2a2a4a;margin-bottom:12px;">
        ...
        ${hasAnyRuna ? runeHTML : '<div style="font-size:0.8rem;color:#6b7280;text-align:center;padding:8px;">No tenés runas equipadas. Conseguilas en la <b style="color:#a78bfa;cursor:pointer;" onclick="Game.classes.closeCharacterScreen();Game.dungeons.showDungeonSelection()">🔮 Cámara Rúnica</b>.</div>'}
